...
export default function render(ctx: ModuleCtx) {
    ...
    const [localComplete, setLocalComplete] = ctx.useState<Record<number, number>>([]);
    ...
    const tasksWithLocalCompletion: Task[] = (donetickTasksQuery.tasks || [])
        .map((task) => {
            const completedAt = localComplete[task.id];
            if (completedAt) {
